...
var db = new DB(
  db_name,
  new DB_Server(db_host, db_port, { auto_reconnect: true, poolSize: 20 }),
  { w: 1 }
);
...
var page = function(req, res, state, ts) {
  db.collection("gumball", function(err, collection) {
    collection
      .find({ serialNumber: "1234998871109" })
...
var handle_post = function(req, res, next) {
  console.log(
    "Post: " + "Action: " + req.body.event + " State: " + req.body.state + "\n"
  );
  ...
  if (diff > 120 || hash1 != hash2) {
    ...
    if (state == "no-coin") page(req, res, "has-coin", ts);
    else page(req, res, state, ts);
